...
namespace MetodosSistemas
{
    class Program
    {   
        static string op; // Variable global para guardar la opcion seleccionada.
        static void Main(string[] args)
        {
            Vector p0; // Vector para los valores iniciales.
            double tol; // Tolerancia a considerar en los metodos.
            int iteraciones;

            while(true) {
                // Primer menu, se muestran los tipos de sistemas al usuario
                ...
                Console.Write("  > ");
                op = Console.ReadLine(); // Se guarda la opcion seleccionada
                op.Replace(" ", ""); // Se eliminan posibles espacios

                if(op.Equals("1")) {
                    Console.Clear();
                    while(true) {
                        // Se muestra un menu de sistemas de ecuaciones lineales para 
                        // resolver por el metodo de Gauss-Jacobi o Gauss-Seidel.
                        ...
                        if(op.Equals("1") || op.Equals("2") || op.Equals("3") || op.Equals("4")) {
                            ...
                            while(true) {
                                ...
                                Console.WriteLine("2) Metodo de Gauss-Seidel");
                                Console.WriteLine("3) Salir");
                                Console.WriteLine("\nSeleccione una opcion: ");
                                Console.Write("  > ");
                                op = Console.ReadLine(); // Se guarada la opcion seleccionada
                                op.Replace(" ", ""); // Se eliminan posibles espacios

                                if(op.Equals("1")) {
                                    Console.WriteLine("\n////// Metodo de Gauss-Jacobi //////");
                                    // Se pide al usuario que ingrese la tolerancia
                                    Console.Write("\nIngrese la tolerancia: ");
                                    tol = Convert.ToDouble(Console.ReadLine());

                                    // Se pide al usuario el no de iteraciones
                                    Console.Write("Ingrese el no de iteraciones: ");
                                    iteraciones = Convert.ToInt32(Console.ReadLine());

                                    // Se ejecuta el metodo de Gauss-Jacobi
                                    ...
                                    // Se ejecuta el metodo de Gauss-Seidel
                                    GaussSeidel(aux, tol, iteraciones);
                                } else if(op.Equals("3")) {
                    ...
                    while(true) {
                        ...
                        if(op.Equals("1") || op.Equals("2")) {
                            ...
                            // metodo de Newton-Raphson.
                            Console.WriteLine("\n--- Solución ---");
                            Vector r = NewtonRaphson(p0, tol, iteraciones);
                            
                            Console.WriteLine($"\n[ x -> {r.vector[0]}, y -> {r.vector[1]}, z -> {r.vector[2]} ]");
                        } else if(op.Equals("3")) {
                            // Regresa al menú anterior
                            Console.Clear();
                            break;
                    ...
                    // Sale del menu principal y termina el programa
                    Console.Clear();
                    break;
                } else {
                    Console.WriteLine($"Error: La opcion '{op}' no existe, intentelo de nuevo ):");
        ...
        // el usuario para el metodo de 
        // Gauss-Jacobi y  Gauss-Seidel.
        static Matriz System(string sistema) {
            Matriz m;
            
            // Si el usuario seleccionó el primer sistema
            if(sistema.Equals("1")) {
            ...
            // Si el usuario seleccionó el segundo sistema
            ...
            // Si el usuario seleccionó el tercer sistema 
            ...
            // Si el usuario seleccionó el cuarto sistema
            else {
                m = new Matriz(new double[,] {
                    { 2, 8, 3, 1 }, 
                    { 0, 2, -1, 4 }, 
        ...
        static Vector VectorResultados(string sistema) {
            ...
            // Si el usuario seleccionó el primer sistema
            if(sistema.Equals("1")) v = new Vector("{3, -2, 5, 4}");
            // Si el usuario seleccionó el segundo sistema
            else if(sistema.Equals("2")) v = new Vector("{15, 11, 6, 6}");
            // Si el usuario seleccionó el tercer sistema
            else if(sistema.Equals("3")) v = new Vector("{0, 2, 3, -2}");
            // Si el usuario seleccionó el cuarto sistema
            else  v = new Vector("{-2, 4, 3, 5}");
        ...
        static Matriz J(Vector v) {
            ...
            if(op.Equals("1")) {
                ...
                J = new Matriz(new double[,]{
                    {3, z * Math.Sin(y * z), y * Math.Sin(y * z)},
                    {8 * x, 2-1250 * y, 0},
                    {-x * (Math.Exp(-x * y)) * (x * y - 2), Math.Pow(-x, 3) * Math.Exp(-x * y), 20}
                });
        ...
        static void GaussJacobi(string sistema, double tol_, int n) {
            ...
            while(error > tol && itr < Iteraciones)
            {
                ...
                Console.WriteLine($"\n[ x -> {Final.vector[0]}, y -> {Final.vector[1]}, z -> {Final.vector[2]} ]");
        ...
        static void GaussSeidel(string sistema, double tol_, int n){
            Matriz m = System(sistema);
            Vector s = VectorResultados(sistema);
            Console.WriteLine("Matriz original");
            m.Print();
            Console.WriteLine("Solucion esperada");
            s.Print();
            double tol = tol_, error = 100, sumaError;
            int Iteraciones = n;
            int itr = 0;
            
            Vector solucion = new Vector("{ 0, 0, 0, 0 }");
            Matriz matrizDominante = Matriz.Dominante(m);
            Console.WriteLine("Matriz total mente dominante");
            matrizDominante.Print();
            //matriz trangular superior (N)
            Matriz triangularSuperior = Matriz.TriangularSuperior(matrizDominante);
            //inversa de la matriz tringula superior (N^-1)
            Matriz inversa = Matriz.Inverse(triangularSuperior);
            //Matriz trinagular inferior
            Matriz triangularInferior = Matriz.TriangularInferior(matrizDominante);
            //multiplicacion de la inversa de la matriz triangular superior por la solucion esperada
            Vector m1 = Matriz.Mult(inversa,s);
            m1.Print();
            //multiplicacion de la inversa de la matriz triangular superior por la matriz triangular
            Matriz m2 = Matriz.Mult(inversa,triangularInferior);
            m2.Print();
            Vector v = Matriz.Mult(m2,solucion);
            v.Print();
            Vector v1 = Vector.Add(v,m1);
            Console.WriteLine("\n -- Iteración 1 -- ");
            v1.Print();
            while(error > tol && itr < Iteraciones){
                Vector xnew = new Vector(4);
                ...
                //xnew.Print();
                //v.Print();
                Console.WriteLine($"\n[ x -> {v1.vector[0]}, y -> {v1.vector[1]}, z -> {v1.vector[2]} ]");
                //v1.Print();
                sumaError = 0;
                for(int i = 0; i < 4; i++)
                {
                    sumaError = sumaError + Math.Pow((xnew.vector[i] - v1.vector[i]),2);
                }
        ...
        static Vector NewtonRaphson(Vector p0_, double tol, int n) {
            ...
            // de las variables (x, y, z), y el numero de itereaciones
            // realizadas.
            Console.WriteLine("\nIteraciones: " + i);
            Console.WriteLine("\nError: " + dist);
